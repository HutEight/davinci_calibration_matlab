...
%%
% fill in the blank
file_path = '20180507_frozen_data_01/green_frozen.csv'
...
for i=1:length
    ...
    pm = plane(psm1_pts_Polaris_cube((length^0 + (i-1)*(length^2)):(length^2 + (i-1)*(length^2)),1), ...
               psm1_pts_Polaris_cube((length^0 + (i-1)*(length^2)):(length^2 + (i-1)*(length^2)),2), ...
               psm1_pts_Polaris_cube((length^0 + (i-1)*(length^2)):(length^2 + (i-1)*(length^2)),3)); 
    ...
    for n = 1:(length^2)
        ...
        dist_square = ((a*psm1_pts_Polaris_cube(n,1)+b*psm1_pts_Polaris_cube(n,2)+c*psm1_pts_Polaris_cube(n,3)+d)^2)/(a^2 + b^2 + c^2);
